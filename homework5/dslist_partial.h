...
++size_;
    if(head_){
        ...
        //new list with value in the front.
            ...
            Node<T>* newp = new Node<T>();//(head_->elements_)
            head_->prev_=newp;
            newp->next_ = head_;
            head_=newp;
    ...
    head_->num_elements_++;//update num of elements 
...
typename dslist<T>::iterator dslist<T>::erase(iterator itr) {
  ...
  if (itr.ptr_==head_ && head_ == tail_){
      if (itr.ptr_->num_elements_==1){ //delete node
        head_ = tail_ = NULL; //NULL
        delete itr.ptr_;
      }else{ //dont delete node. simple erase
        T arr[NUM_ELEMENTS_PER_NODE];
        for (unsigned int i =0; i < itr.ptr_->num_elements_; i++){
            if (i < itr.offset_){
                arr[i]==itr.ptr_->elements_[i];
            }else if (i >itr.offset_){
                arr[i-1] = itr.ptr_->elements_[i];
            }
        }
        for (unsigned int j = 0; j< NUM_ELEMENTS_PER_NODE; j++){ //COPIES VALUES BACK INTO ELEMENTS
            itr.ptr_->elements_[j] = arr[j];
        }
