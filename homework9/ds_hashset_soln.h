...
//##################################################################################################################
  ...
  private:
    // private constructors for use by the HashTablehset only
    iterator(HashTable * hs) : m_hs(hs), m_index(-1) {}
    iterator(HashTable* hs, int index, hash_list_itr loc)
      : m_hs(hs), m_index(index), m_list_itr(loc) {}
...
pair< iterator, bool > insert(KeyType const& key) {
    const float LOAD_FRACTION_FOR_RESIZE = 1.25; //Occupancy Factor Here 
    ...
    //  If the key is not already in the table
    hash_list_itr p = find( m_table[index].begin(), m_table[index].end(), key );
    if ( p == m_table[index].end() ) {
        m_table[index].push_front( key );
        iterator h_itr( this, index, m_table[index].begin() );
        m_size ++ ;
