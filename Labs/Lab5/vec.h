...
public:   
  ...
  size_t remove_matching_elements(Vec<T>& v, T t);
  void print();
...
template <class T> size_t Vec<T>::remove_matching_elements(Vec<T>& v, T t){
  size_type count = 0;
  size_type s = 0;
...
template <class T> void Vec<T>::print(){
  ...
  for ( int i = 0; i < 10; i++){
 //  t.push_back(i);
   cout<<"m_alloc "<<m_alloc<<std::endl;
   cout<<"m_size "<<m_size<<std::endl;
   cout<<"m_data "<<m_data[i]<<std::endl;
  }
  for (int z = 0; z<10; z++){
