...
bool find_roots(int a, int b, int c, float &root_pos, float &root_neg) {
  ...
  float sqrt_radical = sqrt(radical);  //Fixed radikal to radical
  // compute the two roots
  root_pos = (-b + sqrt_radical) / (2*a); // added root_pos and root_neg of root_1 and root_2 
...
int main() {
  ...
  while (true) {
    ...
    // Check our work...
    if (check_root(my_a,my_b,my_c, root_1) && check_root(my_a,my_b,my_c, root_2)) {  //added bracket at end.
