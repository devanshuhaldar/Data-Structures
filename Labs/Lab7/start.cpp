...
int recursion(int x, int y, std::vector<std::vector<GRID_STATUS>>& blocked_grid){
  ...
  if(x==0 && y==0){
    ...
    return (recursion(x-1,y,blocked_grid) + recursion(x, y-1,blocked_grid));
  }else if (x>0 && y == 0){
    return recursion(x-1,y,blocked_grid);
  }else if (x==0 && y>0){
    return recursion(x,y-1,blocked_grid);
  }
  return 0;
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout<<recursion(start_x,start_y,blocked_grid)<<std::endl;
